...
hyperfine --warmup 2 './target/release/cxt -p ./mock_repo'

echo -e "\n=== 2. Peak RAM & CPU Usage (OS Time) ==="
if [[ "$OSTYPE" == "darwin"* ]]; then
    /usr/bin/time -l ./target/release/cxt -p ./mock_repo > /dev/null
else
    /usr/bin/time -v ./target/release/cxt -p ./mock_repo > /dev/null
fi

echo -e "\n=== 3. Internal Heap Profiling (dhat) ==="
cargo build --release --features dhat-heap
./target/release/cxt -p ./mock_repo > /dev/null
